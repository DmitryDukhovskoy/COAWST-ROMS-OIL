...
!cjw--
      ...
      integer :: ibg3
      integer :: ibg4
!cjw---add trace for gene transcription
      logical :: isoil 
!cjw--
#ifdef CARBON
...
!cjw--
      real(r8), allocatable :: Vp1(:)                !tuning parameters for temp limitation
      real(r8), allocatable :: gnp2(:)               !gene panelty on max growth for bug2
      real(r8), allocatable :: gnp3(:)
      real(r8), allocatable :: gnp4(:)
      real(r8), allocatable :: BgMRs(:)              ! 1/day
      real(r8), allocatable :: BgMRa(:)              ! 1/day
      real(r8), allocatable :: BgMRr(:)              ! 1/day
      real(r8), allocatable :: K_bg2(:)              ! (mmol_N/m3)^2
      real(r8), allocatable :: K_bg3(:)              ! (mmol_N/m3)^2
      real(r8), allocatable :: K_bg4(:)              ! (mmol_N/m3)^2
      real(r8), allocatable :: K_Sat(:)              ! m3/mmol_N
      real(r8), allocatable :: K_Arom(:)             ! m3/mmol_N
      real(r8), allocatable :: K_Resin(:)            ! m3/mmol_N
      real(r8), allocatable :: econsta_S(:)          !energy take-up parameters
      real(r8), allocatable :: econsta_A(:)          !
      real(r8), allocatable :: econsta_R(:)          !
      real(r8), allocatable :: szbg2(:)              !size
      real(r8), allocatable :: szbg3(:)
      real(r8), allocatable :: szbg4(:)
      real(r8), allocatable :: betab(:)              !
      real(r8), allocatable :: betap(:)
      real(r8), allocatable :: betas2(:)
      real(r8), allocatable :: betas3(:)
      real(r8), allocatable :: betas4(:)
!cjw--
      CONTAINS
...
!
      ...
      END IF
!cjw---
      IF (.not.allocated(Vp1)) THEN
        allocate ( Vp1(Ngrids) )
      END IF
      IF (.not.allocated(gnp2)) THEN
        allocate ( gnp2(Ngrids) )
      END IF
      IF (.not.allocated(gnp3)) THEN
        allocate ( gnp3(Ngrids) )
      END IF
      IF (.not.allocated(gnp4)) THEN
        allocate ( gnp4(Ngrids) )
      END IF
      IF (.not.allocated(BgMRs)) THEN
        allocate ( BgMRs(Ngrids) )
      END IF
      IF (.not.allocated(BgMRa)) THEN
        allocate ( BgMRa(Ngrids) )
      END IF
      IF (.not.allocated(BgMRr)) THEN
        allocate ( BgMRr(Ngrids) )
      END IF
      IF (.not.allocated(K_bg2)) THEN
        allocate ( K_bg2(Ngrids) )
      END IF
      IF (.not.allocated(K_bg3)) THEN
        allocate ( K_bg3(Ngrids) )
      END IF
      IF (.not.allocated(K_bg4)) THEN
        allocate ( K_bg4(Ngrids) )
      END IF
      IF (.not.allocated(K_Sat)) THEN
        allocate ( K_Sat(Ngrids) )
      END IF
      IF (.not.allocated(K_Arom)) THEN
        allocate ( K_Arom(Ngrids) )
      END IF
      IF (.not.allocated(K_Resin)) THEN
        allocate ( K_Resin(Ngrids) )
      END IF
      IF (.not.allocated(econsta_S)) THEN
        allocate ( econsta_S(Ngrids) )
      END IF
      IF (.not.allocated(econsta_A)) THEN
        allocate ( econsta_A(Ngrids) )
      END IF
      IF (.not.allocated(econsta_R)) THEN
        allocate ( econsta_R(Ngrids) )
      END IF
      IF(.not.allocated(szbg2)) THEN
        allocate(szbg2(Ngrids))
      END IF
      IF(.not.allocated(szbg3)) THEN
        allocate(szbg3(Ngrids))
      END IF
      IF(.not.allocated(szbg4)) THEN
        allocate(szbg4(Ngrids))
      END IF
      IF(.not.allocated(betab)) THEN
        allocate(betab(Ngrids))
      END IF
      IF(.not.allocated(betap)) THEN
        allocate(betap(Ngrids))
      END IF
      IF(.not.allocated(betas2)) THEN
        allocate(betas2(Ngrids))
      END IF
      IF(.not.allocated(betas3)) THEN
        allocate(betas3(Ngrids))
      END IF
      IF(.not.allocated(betas4)) THEN
        allocate(betas4(Ngrids))
      END IF
!cjw---
!
